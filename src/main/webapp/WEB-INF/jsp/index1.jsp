        ...
        <ul class="layui-nav layui-layout-right" lay-filter="layadmin-layout-right">
          ...
          <li class="layui-nav-item" lay-unselect>
            ...
            <dl class="layui-nav-child">
              <dd><a lay-href="${pageContext.request.contextPath}/user/toPersonalInfo">基本资料</a></dd>
              <dd><a lay-href="${pageContext.request.contextPath}/user/updatePwd">修改密码</a></dd>
              <hr>
              <dd style="text-align: center;"><a href="${pageContext.request.contextPath}/login/loginout">退出</a></dd>
            </dl>
          </li>
          <li class="layui-nav-item" lay-unselect>
            <a href="javascript:;">
              <cite>${sessionScope.role.roleName}</cite>
            </a>
          </li>
      ...
      <div class="layui-side layui-side-menu">
        <div class="layui-side-scroll">
          ...
          <ul class="layui-nav layui-nav-tree" lay-filter="test">
            <c:forEach items="${sessionScope.menuList}" var="ml">
              <c:if test="${ml.level==1}">
                <li class="layui-nav-item layui-bg-cyan">
                  <a data-toggle="sidenav" data-id-key="targetid">${ml.menuName}</a>
                  <dl class="layui-nav-child">
                    <c:forEach items="${sessionScope.menuList}" var="ml2">
                      <c:if test="${ml2.level==2&&ml2.parentId==ml.menuId}">
                        <dd>
                          <c:choose>
                            <c:when test="${ml2.isChild}">
                              <a href="javascript:;">${ml2.menuName}</a>
                              <dl class="layui-nav-child">
                                <c:forEach items="${sessionScope.menuList}" var="ml3">
                                  <c:if test="${ml3.level==3&&ml3.parentId==ml2.menuId}">
                                    <dd><a lay-href="${ml3.menuSrc}">${ml3.menuName}</a></dd>
                                  </c:if>
                                </c:forEach>
                              </dl>
                            </c:when>
                            <c:otherwise>
                              <a lay-href="${pageContext.request.contextPath}${ml2.menuSrc}">${ml2.menuName}</a>
                            </c:otherwise>
                          </c:choose>
                        </dd>
                      </c:if>
